...
namespace Writing {
	void GLM3ToFile(std::ofstream& outFile, glm::vec3 const& vec) {
		outFile.write(reinterpret_cast<const char*>(&vec.x), sizeof(float));
		outFile.write(reinterpret_cast<const char*>(&vec.y), sizeof(float));
		outFile.write(reinterpret_cast<const char*>(&vec.z), sizeof(float));
	}
	...
	void UInt64ToFileSwapEndian(std::ofstream& outFile, uint64_t const* value) {
		...
		outFile.write(reinterpret_cast<const char*>(&outValue), sizeof(uint64_t));
	}
	...
	void UIntToFileSwapEndian(std::ofstream& outFile, uint32_t const* value) {
		...
		outFile.write(reinterpret_cast<const char*>(&outValue), sizeof(uint32_t));
	}
	...
	void IntToFileSwapEndian(std::ofstream& outFile, int const* value) {
		...
		outValue = ((outValue & 0xFF) << 24) | (((outValue >> 8) & 0xFF) << 16) | (((outValue >> 16) & 0xFF) << 8) | ((outValue >> 24) & 0xFF);
		outFile.write(reinterpret_cast<const char*>(&outValue), sizeof(int));
	}
	void FloatToFileSwapEndian(std::ofstream& outFile, float const* value) {
		uint32_t outValue;
		memcpy(&outValue, value, sizeof(float));
		outValue = ((outValue & 0xFF) << 24) | (((outValue >> 8) & 0xFF) << 16) | (((outValue >> 16) & 0xFF) << 8) | ((outValue >> 24) & 0xFF);
		outFile.write(reinterpret_cast<const char*>(&outValue), sizeof(float));
	}
